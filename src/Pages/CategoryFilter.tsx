...
const CategoryFilter = () => {
  ...
  const title = useParams<{ title: keyof typeof DummyData }>().title || "AllItems";
  const productsToShow = DummyData[title] || [];

  const uniqueCategories = useMemo(() => {
    const categories = new Set(productsToShow.map((item:any) => item.category));
    return Array.from(categories);
  }, [productsToShow]);

  const prices = productsToShow.map((p:any) => p.offerPrice || p.price || 0);
